...
. Verify Enonic app hs **the `movie-details` part descriptor** at - same procedure as before...
+
...
----
+
. **Redeploy** the Enonic app if you made any changes (happens automatically if you started `enonic project dev`).
+
...
+
. **Activate the context panel** by pressing its icon in the top right screen corner, select the `Page` extension, then "Main page" descriptor in dropdown and **add the "Movie details" part** to the page.
+
...
+
. In the page tab in the form on the left, **select the page (root) component**.
+
