...
The content type keys must be _full XP content type_ strings, with app name and type name - e.g. `"base:folder"`, `"tutorial.nxp:my-content-type"`, etc.
...
+
Make a note of the full content type name: a colon-separated string with both <<enonic-setup#new-project, app>> and <<enonic-setup#xp-content, type>> name, for example `'tutorial.nxp:my-content-type'`.
+
